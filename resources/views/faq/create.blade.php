...
<div class="content-wrapper">
  ...
  <div class="page-header">
      <h1><i class="glyphicon glyphicon-edit"></i> Faq / Create</h1>
  </div>
  </section>
    <!-- Main content -->
            ...
            <form action="{{ route('faq.store') }}" enctype="multipart/form-data" method="POST">
                ...
                <div class="form-group @if($errors->has('title')) has-error @endif">
                    ...
                    <div class="form-group @if($errors->has('file1')) has-error @endif">
                       <label for="picture-field">File 1</label>
                    <input type="file" id="picture-field" name="file1" accept="application/pdf,image/x-png,image/gif,image/jpeg" class="form-control" value="{{ old("file1") }}"/>
                       @if($errors->has("file1"))
                        <span class="help-block">{{ $errors->first("file1") }}</span>
                       @endif
                    </div>
                    <div class="form-group @if($errors->has('file2')) has-error @endif">
                       <label for="picture-field">File 2</label>
                    <input type="file" id="picture-field" name="file2" accept="application/pdf,image/x-png,image/gif,image/jpeg" class="form-control" value="{{ old("file2") }}"/>
                       @if($errors->has("file2"))
