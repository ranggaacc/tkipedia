    ...
    <div class="container">
      <div class="search-result">
        <div class="card clearfix">
          <a class="card__title">{{$informasis->title}}</a>
          <i class="glyphicon glyphicon-calendar"></i> {{date('j F Y', strtotime($informasis->tanggal))}}
          <i class="glyphicon glyphicon-eye-open"></i> {{$informasis->view}}
          <p class="card__p">
              @if($informasis['picture'] != null)
                <img src="{{ asset($informasis->picture) }}" alt="" class="box__img img-responsive">
              @else
                <img src="{{ asset("/upload/images/profil/noimage.png") }}" class="box__img img-responsive">
              @endif

            {!!$informasis->description!!}
          </p>
          <?php $splitted1 = preg_split('/\//', $informasis->file1); ?>
          <p class="card__p">Link Download 1 <a href="{{asset($informasis->file1) }}" download>{{$splitted1[sizeof($splitted1) - 1]}}</a></p>
          <?php $splitted2 = preg_split('/\//', $informasis->file2); ?>
          <p class="card__p">Link Download 2 <a href="{{asset($informasis->file2) }}" download>{{$splitted2[sizeof($splitted2) - 1]}}</a></p>
        </div>
      </div>
    </div>
  </section>
